...
if [ ! -f "$SUMMARY_FILE" ]; then
    echo "GlobalPlanner,LocalPlanner,Status,Time(s),Distance(m),CPU(%),Memory(%)" > "$SUMMARY_FILE"
fi

for global in "${GLOBAL_PLANNERS[@]}"; do
    for local in "${LOCAL_PLANNERS[@]}"; do
        echo ""
        echo ">>> Starting Benchmark: Global=$global | Local=$local"
        ...
        while true; do
            ...
            if [ $ELAPSED -gt $TIMEOUT ]; then
                echo "Timeout waiting for simulation (Global: $global, Local: $local)!"
                ./killpro.sh
                exit 1
            fi
            sleep 2
        ...
        roslaunch plannie benchmark.launch planner:="${global}_${local}" goal_x:=$GOAL_X goal_y:=$GOAL_Y summary_file:=$SUMMARY_FILE
        
        echo ">>> Finished: ${global}_${local}"
        echo "------------------------------------------"
